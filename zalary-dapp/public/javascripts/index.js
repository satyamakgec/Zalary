angular.module('angularApp', ['ui.bootstrap'])
  .filter('updateExpiry', function() {

    return function(input) {

      input = moment(input).add(12, 'M').format("MMM/D/YYYY")
  ...
  .controller('indexCtrl', function($scope, $http, $window) {
    ...
    $scope.potentialInvestors = function(address) {
      $http.get("/api/whitelist/" + $scope.token)
      ...
      .then(function(response) {
        ...
        let result = response.data.map(a => a.Kyc_check.Investor.address_country);

        var countries = result.filter(onlyUnique);

        var countryList = [{"name" : "All", "value" : "All"}]
        $scope.countrySelect = countryList[0].value;

        var arrayLength = countries.length;
        for (var i = 0; i < arrayLength; i++) {

          countryList.push({"name" : countries[i], "value" : countries[i]})

        }
        //countryList.push({"name" : "All", "value" : "All"})
 ...
 function getDayClass(data) {
   ...
   return '';
    ...
    $scope.addToWhitelist = function(record) {
      ...
      record.i = {issuerAddress: $scope.issuerAddress}
